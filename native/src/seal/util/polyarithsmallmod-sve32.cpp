...
namespace seal
{
    namespace util
    {
            ...
            CoeffIter result)
        {
            ...
            {
                const uint64_t *poly_ptr = poly.ptr();
                uint64_t *result_ptr = result.ptr();
                const uint64_t p = modulus.value();
                const svuint64_t sv_mod = svdup_n_u64(p);
                const svuint64_t sv_op = svdup_n_u64(scalar.operand);
                const svuint64_t sv_q = svdup_n_u64(scalar.quotient);
                ...
                for (; i < full; i += N)
                {
                    svuint64_t v_x = svld1_u64(ptrue, poly_ptr + i);
                    svuint64_t v_hi = sve32_mulhi_u64(ptrue, v_x, sv_q);
                    svuint64_t v_lo = svmul_u64_x(ptrue, v_x, sv_op);
                    v_lo = svmls_u64_x(ptrue, v_lo, sv_mod, v_hi);
                    svbool_t ge = svcmpge_u64(ptrue, v_lo, sv_mod);
                    v_lo = svsub_u64_m(ge, v_lo, sv_mod);
                    svst1_u64(ptrue, result_ptr + i, v_lo);
                }
                if (i < coeff_count)
                {
                    svbool_t pg = svwhilelt_b64_u64(i, coeff_count);
                    svuint64_t v_x = svld1_u64(pg, poly_ptr + i);
                    svuint64_t v_hi = sve32_mulhi_u64(pg, v_x, sv_q);
                    svuint64_t v_lo = svmul_u64_x(pg, v_x, sv_op);
                    v_lo = svmls_u64_x(pg, v_lo, sv_mod, v_hi);
                    svbool_t ge = svcmpge_u64(pg, v_lo, sv_mod);
                    v_lo = svsub_u64_m(ge, v_lo, sv_mod);
                    svst1_u64(pg, result_ptr + i, v_lo);
                }
            }
#else
            SEAL_ITERATE(iter(poly, result), coeff_count, [&](auto I) {
