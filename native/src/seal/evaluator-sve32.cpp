...
namespace seal
{
    ...
    namespace
    {
        ...
        // SVE helper: 128-bit addition (lo, hi) += (addend_lo, addend_hi)
        ...
        // SVE vectorized Barrett reduction for switch_key_inplace lazy accumulator.
        ...
        static void sve_accumulate_and_reduce(
            ...
            size_t count, const Modulus &modulus, bool do_reduce)
        {
            ...
            const svuint64_t sv_mod = svdup_n_u64(modulus.value());
            const svuint64_t sv_ratio0 = svdup_n_u64(modulus.const_ratio()[0]);
            const svuint64_t sv_ratio1 = svdup_n_u64(modulus.const_ratio()[1]);
            const size_t full = (count / N) * N;
            ...
            if (do_reduce)
            {
                ...
                for (; i < full; i += N)
                {
                    svuint64_t v_op = svld1_u64(ptrue, t_operand + i);
                    svuint64_t v_key = svld1_u64(ptrue, key + i);
                    svuint64_t prod_lo = svmul_u64_x(ptrue, v_op, v_key);
                    svuint64_t prod_hi = sve32_mulhi_u64(ptrue, v_op, v_key);

                    svuint64x2_t acc_pair = svld2_u64(ptrue, acc + 2 * i);
                    svuint64_t acc_lo = svget2_u64(acc_pair, 0);
                    svuint64_t acc_hi = svget2_u64(acc_pair, 1);
                    sve_add_u128(acc_lo, acc_hi, prod_lo, prod_hi, ptrue);

                    svuint64_t res = sve_barrett_reduce_128(acc_lo, acc_hi, sv_mod, sv_ratio0, sv_ratio1, ptrue);
                    svuint64x2_t out_pair = svcreate2_u64(res, svdup_n_u64(0));
                    svst2_u64(ptrue, acc + 2 * i, out_pair);
                }
                // Tail
                if (i < count)
                {
                    svbool_t pg = svwhilelt_b64_u64(i, count);
                    svuint64_t v_op = svld1_u64(pg, t_operand + i);
                    svuint64_t v_key = svld1_u64(pg, key + i);
                    svuint64_t prod_lo = svmul_u64_x(pg, v_op, v_key);
                    svuint64_t prod_hi = sve32_mulhi_u64(pg, v_op, v_key);

                    svuint64x2_t acc_pair = svld2_u64(pg, acc + 2 * i);
                    svuint64_t acc_lo = svget2_u64(acc_pair, 0);
                    svuint64_t acc_hi = svget2_u64(acc_pair, 1);
            ...
            else
            {
                ...
                for (; i < full; i += N)
                {
                    ...
                    svuint64x2_t out_pair = svcreate2_u64(acc_lo, acc_hi);
                    svst2_u64(ptrue, acc + 2 * i, out_pair);
                }
                // Tail
                if (i < count)
        ...
        // SVE vectorized final Barrett reduction on stride-2 accumulator
        // acc layout: [lo0, hi0, lo1, hi1, ...], count pairs
        // result: each lo is Barrett-reduced mod modulus, written to output
        static void sve_barrett_reduce_accumulator(
                ...
                SEAL_ITERATE(iter(shifted_in1_iter, shifted_reversed_in2_iter), steps, [&](auto J) {
                    SEAL_ITERATE(iter(J, base_iter, shifted_out_iter), base_size, [&](auto K) {
#ifdef __ARM_FEATURE_SVE
                        {
                            ...
                            const uint64_t p = get<1>(K).value();
                            const svuint64_t sv_mod = svdup_n_u64(p);
                            const svuint64_t sv_r0 = svdup_n_u64(get<1>(K).const_ratio()[0]);
                            const svuint64_t sv_r1 = svdup_n_u64(get<1>(K).const_ratio()[1]);

                            // Fused dyadic product + accumulate: out = (out + in1*in2) mod p
                            const uint64_t N = svcntd();
                            const svbool_t ptrue = svptrue_b64();
                            const size_t full = (coeff_count / N) * N;
                            size_t k = 0;
                            for (; k < full; k += N)
                            {
                                svuint64_t a = svld1_u64(ptrue, in1_ptr + k);
                                svuint64_t b = svld1_u64(ptrue, in2_ptr + k);
                                svuint64_t o = svld1_u64(ptrue, out_ptr + k);
                                svuint64_t z_lo = svmul_u64_x(ptrue, a, b);
                                svuint64_t z_hi = sve32_mulhi_u64(ptrue, a, b);
                                svuint64_t prod = sve_barrett_reduce_128(z_lo, z_hi, sv_mod, sv_r0, sv_r1, ptrue);
                                svuint64_t sum = svadd_u64_x(ptrue, o, prod);
                                svbool_t ge = svcmpge_u64(ptrue, sum, sv_mod);
                                sum = svsub_u64_m(ge, sum, sv_mod);
                                svst1_u64(ptrue, out_ptr + k, sum);
                            }
                            // Tail
                            if (k < coeff_count)
                            {
                                svbool_t pg = svwhilelt_b64_u64(k, coeff_count);
                                svuint64_t a = svld1_u64(pg, in1_ptr + k);
                                svuint64_t b = svld1_u64(pg, in2_ptr + k);
                                svuint64_t o = svld1_u64(pg, out_ptr + k);
                                svuint64_t z_lo = svmul_u64_x(pg, a, b);
                                svuint64_t z_hi = sve32_mulhi_u64(pg, a, b);
                                svuint64_t prod = sve_barrett_reduce_128(z_lo, z_hi, sv_mod, sv_r0, sv_r1, pg);
                                svuint64_t sum = svadd_u64_x(pg, o, prod);
                                svbool_t ge = svcmpge_u64(pg, sum, sv_mod);
                                sum = svsub_u64_m(ge, sum, sv_mod);
                                svst1_u64(pg, out_ptr + k, sum);
                            }
                        }
#else
        ...
        MemoryPoolHandle pool) const
    {
        ...
        SEAL_ITERATE(iter(size_t(0)), rns_modulus_size, [&](auto I) {
            ...
            SEAL_ITERATE(iter(accumulator_iter, t_poly_prod_iter), key_component_count, [&](auto K) {
                ...
                {
                    sve_barrett_reduce_accumulator(
            ...
            if (scheme == scheme_type::bgv)
            {
                ...
                SEAL_ITERATE(iter(I, key_modulus, modswitch_factors, key_ntt_tables), decomp_modulus_size, [&](auto J) {
...
#ifdef __ARM_FEATURE_SVE
                    {
                        ...
                        const uint64_t p = get<1>(J).value();
                        svuint64_t sv_mod = svdup_n_u64(p);

                        // out = (a + b) mod p (a,b in [0,p))
                        const uint64_t N = svcntd();
                        const svbool_t ptrue = svptrue_b64();
                        const size_t full = (coeff_count / N) * N;
                        size_t k = 0;
                        for (; k < full; k += N)
                        {
                            svuint64_t va = svld1_u64(ptrue, a_ptr + k);
                            svuint64_t vb = svld1_u64(ptrue, b_ptr + k);
                            svuint64_t sum = svadd_u64_x(ptrue, va, vb);
                            svbool_t ge = svcmpge_u64(ptrue, sum, sv_mod);
                            sum = svsub_u64_m(ge, sum, sv_mod);
                            svst1_u64(ptrue, b_ptr + k, sum);
                        }
                        if (k < coeff_count)
                        {
                            svbool_t pg = svwhilelt_b64_u64(k, coeff_count);
                            svuint64_t va = svld1_u64(pg, a_ptr + k);
                            svuint64_t vb = svld1_u64(pg, b_ptr + k);
                            svuint64_t sum = svadd_u64_x(pg, va, vb);
                            svbool_t ge = svcmpge_u64(pg, sum, sv_mod);
                            sum = svsub_u64_m(ge, sum, sv_mod);
                            svst1_u64(pg, b_ptr + k, sum);
                        }
                    }
#else
